...
from analysis.utils import get_gender_df, load_demographic_data, no_bs
from dataloader.dataloader import DISEASE_LABELS_CHE, DISEASE_LABELS_NIH
...
def plotting_all(args):
    ...
    if args.dataset == "both":
        ...
        if args.dataset == "NIH":
            ...
            disease_list = DISEASE_LABELS_CHE
    ...
    fig, axes = plt.subplots(
        nrows=nrows, ncols=ncols, figsize=(ncols * 4, nrows * 4), dpi=100
    )
    ...
    for i, each_d in enumerate(disease_list):
        for j, each_ds in enumerate(datasets):
            ...
            axes[j_new][i_new].set_title(
                "{}".format(each_d), fontsize=font_size, weight="bold"
            )
            ...
            df["TrainOnFemalePerc"] = df["exp_suffix"].apply(
                lambda x: 0
            ...
            df["TestOn"] = df["exp_suffix"].apply(
                lambda x: "Test on Male" if "Test on M" in x else "Test on Female"
            )
            df = df[df["rs"] < 10]
            # print(df.shape)
            sns.boxplot(
...
def re_store_results(
    args, dataset, disease_label_list, list_rs, female_perc_in_training_sets
):
    for d in disease_label_list:  # disease_labels:
        ...
        for f_per in female_perc_in_training_sets:
            ...
            for rs in tqdm(list_rs):
                ...
                try:
                    pred_test = pd.read_csv(
                        data_dir
                        + run_config
                        + "/predictions.test.version_{}.csv".format(version_no)
                    )
                    # load demographic data according to dataset
                    df_test = pd.read_csv(
                        data_dir
                        + run_config
                        + "/test.version_{}.csv".format(version_no)
                    )
                except:
                    continue
                ...
                assert (
                    df_test.shape[0] == pred_test.shape[0]
                ), "df_test:{},preds:{}".format(df_test.shape, pred_test.shape)
            ...
            gender_df_this_f_perc["exp_suffix"] = gender_df_this_f_perc[
                "exp_suffix"
            ].apply(lambda x: trainon + " " + x)
        ...
        gender_df_all = pd.concat(
            [result_varioustrain[0], result_varioustrain[1], result_varioustrain[2]]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--run_dir", default="/work3/ninwe/run/", help="your run dir.", type=str
    )
    parser.add_argument(
        "-f",
        "--suffix",
        default="",
        help="any other hyper-parameters used when trainning, need to be manually set up for now",
        type=str,
    )
    ...
    if len(disease_label_list) == 1 and disease_label_list[0] == "all":
        disease_label_list = (
            DISEASE_LABELS_CHE if args.dataset == "chexpert" else DISEASE_LABELS_NIH
        )
    ...
    if args.dataset == "NIH" or args.dataset == "both":
        ...
        re_store_results(
            args, "NIH", disease_label_list, list_rs, female_perc_in_training_sets
        )
    ...
    if args.dataset == "chexpert" or args.dataset == "both":
        ...
        re_store_results(
            args, "chexpert", disease_label_list, list_rs, female_perc_in_training_sets
        )
