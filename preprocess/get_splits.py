...
def sampling_ds(dataset: str, label: str, rs: int, npp: int, out_dir: str):
    ...
    for f_per in [0, 50, 100]:
        ...
        data = ChestXRayModule(
            img_data_dir="/work3/ninwe/dataset/{}/preproc_224x224/".format(dataset),
    ...
    if args.dataset == "chexpert" or args.dataset == "all":
        ...
        for each_d in loop_list:
            ...
            for i in tqdm(np.arange(rs_min, rs_max)):
                print("-" * 30 + "rs:{}".format(i))
                sampling_ds(
                    dataset="chexpert", label=each_d, rs=i, npp=1, out_dir=out_dir
                )
